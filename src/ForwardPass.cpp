...
cl_int NeuralNetwork::ForwardPass(const std::array<float, BxI>& inputs) {
    ...
    if (err != CL_SUCCESS) {
        std::cout << "Failed to create input matrix: " << err << " (" << FILE_NAME(__FILE__) << " > NeuralNetwork::ForwardPass)\n";
        return err;
    }
