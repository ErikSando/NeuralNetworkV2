...
namespace Matrix {
    /**
     * Allocate device memory for a matrix, returns CL error code (0 means sucess)
     * host_mat - host array
    ...
    /**
     * Calls clReleaseMemObject on the given device memory.
     */
    void Destroy(cl_mem& dev_mem);

    /**
     * Performs C = AB, returns CL error code (0 means sucess)
    ...
    /**
     * Performs C = A + B, returns CL error code (0 means sucess)
     * dmA - device memory for matrix A
     * dmB - device memory for matrix B
     * dmC - device memory for matrix C
     ...
     */
    cl_int Add(
        Kernel& kernel, const cl_mem& dmA, const cl_mem& dmB, cl_mem& dmC,
        /*float* C,*/ const size_t size
    );

    /**
     * Multiply the matrix by a scalar value, returns CL error code (0 means success)
    ...
    /**
     * Copy device memory buffer into host memory buffer, returns CL error code (0 means success)
    ...
    /**
     * Copy host memory buffer into device memory buffer, returns CL error code (0 means success)
    ...
    /**
     * Sets all values in the matrix to the specified number, returns CL error code (0 means success)
     * mat - device memory of the matrix
     * size - size of the matrix (rows * columns)
     ...
     * Returns CL error code (0 means success)
     * mat - device memory of the matrix
     * size - size of the matrix (rows * columns)
     * min - lower bound
     * max - upper bound
